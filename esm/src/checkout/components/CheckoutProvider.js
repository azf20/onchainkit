...
import '../../../node_modules/@wagmi/core/dist/esm/exports/index.js';
import { createContext, useContext, useState, useRef, useCallback, useEffect } from 'react';
import { base } from 'viem/chains';
import { useAccount, useConnect, useSwitchChain, useWriteContract, useConfig, useWaitForTransactionReceipt } from 'wagmi';
...
import { readContract } from '../../../node_modules/@wagmi/core/dist/esm/actions/readContract.js';
const emptyContext = {};
...
function CheckoutProvider({
  ...
  const transactionHash = callsData?.receipts?.[0]?.transactionHash || (useExperimental ? undefined : transactionId); // In regular flow, transactionId will be the hash of the second transaction
  ...
  useEffect(() => {
    if (writeContractsStatus === 'pending' || writeContractStatus === 'pending') {
      updateLifecycleStatus({
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      for (const contract of contractsRef.current || []) {
        ...
        if (contract.functionName === 'approve' && contract.args && contract.args.length === 2 && typeof contract.args[1] === 'bigint') {
  ...
  }, [address, appearance, chainId, chargeId, connectAsync, errorMessage, fetchData, isConnected, useExperimental, isSponsored, lifecycleStatus.statusName, paymaster, switchChainAsync, updateLifecycleStatus, wagmiConfig, writeContractsAsync, writeContractRegular]);
